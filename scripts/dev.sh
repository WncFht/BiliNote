...
BACKEND_CMD="${BILINOTE_BACKEND_CMD:-exec uv run python main.py}"
FRONTEND_CMD="${BILINOTE_FRONTEND_CMD:-exec pnpm dev}"
FRONTEND_PREVIEW_PORT="${BILINOTE_FRONTEND_PREVIEW_PORT:-3015}"
FRONTEND_PREVIEW_CMD="${BILINOTE_FRONTEND_PREVIEW_CMD:-pnpm build && exec pnpm exec vite preview --host 0.0.0.0 --port $FRONTEND_PREVIEW_PORT}"
