...
if platform.system() == "Darwin":
    try:
        from app.transcriber.mlx_whisper_transcriber import MLX_WHISPER_INSTALLED, MLXWhisperTranscriber

        MLX_WHISPER_AVAILABLE = MLX_WHISPER_INSTALLED
        if MLX_WHISPER_AVAILABLE:
            logger.info("MLX Whisper 可用，已导入")
        else:
            logger.warning("MLX Whisper 模块存在，但 mlx_whisper 依赖未安装")
    except ImportError:
